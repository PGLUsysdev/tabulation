...
if (count($event)) {
	...
	$sql = "SELECT portions.id, portions.description, portions.winners, '".$event[0]['description']."' tabulation_event, portions.event_chairman, portions.event_tabulator FROM portions WHERE portions.event_id = ".$event[0]['id']." AND portions.is_shown = 1";
					...
					if (count($criterion_score)) {
						...
						foreach ($criterion_score as $cs) {
							...
							foreach ($contestants_all_judges[$ic]['scores'] as $iopc => $opc) {
								...
								if ($cs['score_criterion'] == $opc['id']) {
									
									$contestants_all_judges[$ic]['scores'][$iopc]['score'] += $cs['score'];
						...
						foreach ($contestants_all_judges[$ic]['scores'] as $iopc => $opc) {
							...
							if ($criterion_score['score_criterion'] == $opc['id']) {
								
								$contestants_all_judges[$ic]['scores'][$iopc]['score'] += $criterion_score['score'];
	...
	# for rounds
	$sql = "SELECT rounds.id, rounds.description, '".$event[0]['description']."' tabulation_event, rounds.winners, rounds.method, rounds.event_chairman, rounds.event_tabulator FROM rounds WHERE rounds.event_id = ".$event[0]['id']." AND rounds.is_shown = 1";
